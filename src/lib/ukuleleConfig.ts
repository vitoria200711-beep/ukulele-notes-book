...
export function isNoteMatch(
  ...
  // Erro comum em detecção de pitch: oitava (freq *2 ou /2).
  // Vamos escolher a melhor aproximação entre expected, expected*2 e expected/2.
  const candidates = [expectedFrequency, expectedFrequency * 2, expectedFrequency / 2];
  let bestAbsCents = Number.POSITIVE_INFINITY;

  for (const c of candidates) {
    const cents = 1200 * Math.log2(detectedFrequency / c);
    const abs = Math.abs(cents);
    if (abs < bestAbsCents) bestAbsCents = abs;
  }

  if (bestAbsCents <= toleranceCents / 2) return 'correct';
  if (bestAbsCents <= toleranceCents) return 'close';
  return 'wrong';
}
