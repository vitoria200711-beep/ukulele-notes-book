...
// IMPORTANTE:
// - No app, usamos stringIndex: 0=A, 1=E, 2=C, 3=G
// - No ukuleleConfig, o stringIndex é: 0=G, 1=C, 2=E, 3=A
// Então precisamos mapear antes de calcular frequência/nota esperada.
function appStringToConfigString(appStringIndex: number): number {
  // 0(A)->3, 1(E)->2, 2(C)->1, 3(G)->0
  return 3 - appStringIndex;
}
...
export function PracticeMode({ open, onOpenChange, songTitle, notes }: PracticeModeProps) {
  ...
  const startPracticeListening = async () => {
    // Mesmo perfil do afinador: responde mesmo longe do alvo (evita ficar em “Aguardando”).
  ...
  const noteStatuses = useMemo(() => {
    ...
    notes.forEach(note => {
      ...
      const expectedFreq = getFrequencyForPosition(
        ukuleleType,
        appStringToConfigString(note.string),
        note.fret
      );
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Button
              onClick={isListening ? stopListening : startPracticeListening}
